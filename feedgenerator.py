        ...
        for item in self.items:
            ...
            if len(item['enclosure']) > 0:
                for enclosure in item['enclosure']:
                    if enclosure.mime_type == u'audio/mpeg':
                        handler.addQuickElement(u"enclosure", '',
                            {u"url": enclosure.url, u"length": enclosure.length,
                                u"type": enclosure.mime_type})
                    else:
                        #handler.addQuickElement(u"media:content", '', {u"url": enclosure.url, u"type": enclosure.mime_type})
                        handler.addQuickElement(u"media:thumbnail", '', {u"url": enclosure.url,  
                            u'xmlns:media':u'http://search.yahoo.com/mrss/', 
                            u"width": u"75", u"height":"75"})
